...
- 2026-03-29: **SPEC-effects-category-c** implemented. 4 column behaviour effects: (1) gravity wells — `aClusterCenter` vec2 attribute (two-pass centroid in `buildGeometry()`), XZ `colCenter` displacement with `isLocked` guard; (2) perspective convergence — clip-space X skew toward `uPerspectiveWorldX` inside `camDist3D>=1.5` block; (3) Morse flicker — `vAlpha` modulated by per-column hash-derived on/off pattern with `isLocked` guard; (4) spiral formation — XZ orbital rotation of `colCenter` using `aSpawnTheta` as phase winding, `isLocked` guard. 9 uniforms, 6 handle methods, 9 sliders in matrix-3d.html.
